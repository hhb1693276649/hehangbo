...
from flask import request, session, render_template, redirect, url_for, current_app, g, abort, jsonify
...
from utils.image_storage import upload_img
...
def login():
    ...
    if not all([username, password]):
        return render_template('admin/login.html',errmsg='用户密码不完整')
    # 判断超级管理员是否存在
    try:
...
def news_edit_detail():
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errrno=RET.PARAMERR, errmsg=error_map[RET.PARAMERR])
    # 取出新闻模型
    try:
        news = News.query.get(news_id)
...
def news_type():
    ...
    if id:
        ...
        try:
            ...
            current_app.logger.error(e)
            return jsonify(errno=RET.DBERR, errmsg=RET.DBERR)
        if not category:
            return jsonify(errno=RET.NODATA, errmsg=error_map[RET.NODATA])
        category.name = name
    else:  # 增加分类
